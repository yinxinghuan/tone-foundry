...
export function ModularSetNeckNeckSvg({prefix,neck}:{prefix:string;neck:ModularSetNeckId}) {
  ...
  return <g className="tf-module-neck" data-guitar-module={`set-neck-neck-${neck}`}>
    ...
    <g data-guitar-part="three-aside-tuner-backs">
      {GEO.tunerPoints.map(([x,y],index)=>{const side=index<3?-1:1;return <g key={`${x}-${y}`}><path d={`M${x+side*3} ${y} H${x+side*23}`} stroke="#767873" strokeWidth="5.5"/><path d={`M${x+side*20} ${y-9} Q${x+side*35} ${y} ${x+side*20} ${y+9} L${x+side*10} ${y+6} L${x+side*10} ${y-6} Z`} fill={`url(#${prefix}-set-nickel)`} stroke="#555954" strokeWidth=".8"/><path d={`M${x+side*25} ${y-5} Q${x+side*33} ${y} ${x+side*25} ${y+5}`} fill="none" stroke="#f1eee4" strokeWidth=".7" opacity=".72"/></g>})}
    </g>
    ...
    {GEO.tunerPoints.map(([x,y])=><g key={`${x}-${y}-front`} data-guitar-part="three-aside-tuner"><circle cx={x} cy={y} r="8.8" fill={`url(#${prefix}-set-nickel)`} stroke="#555954" strokeWidth=".72"/><polygon points={`${x},${y-5} ${x+4.4},${y-2.5} ${x+4.4},${y+2.5} ${x},${y+5} ${x-4.4},${y+2.5} ${x-4.4},${y-2.5}`} fill="#8c8f89"/><circle cx={x} cy={y} r="2.1" fill="#555854"/><circle cx={x-1.7} cy={y-2} r="1.2" fill="#fff" opacity=".65"/></g>)}
