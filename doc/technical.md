...
- `BuildRun` 状态机包含 `start → sealed → choose → tone → complete → riff / collection / wall / detail`，并提供仅从调音页进入的 `refit` 回看状态。玩家入口不再包裹工作区导航或技术状态栏；制琴中只常驻五个无文字进度点。每局随机平台，依次完成琴体、琴颈、拾音器、琴桥、饰面五步；候选固定 2–3 张且不可刷新。每步揭示的候选会存入本局 `choiceBank`，供调音页安全地重新试装，不产生新抽取。
...
- 制作预览只保留试听与触摸查看器的备用缩放控制；完整的 Clean / Drive、六维音色与比较句只在五部件完成后的全屏 `tone` 状态出现。调音页可回看本局已揭示的候选，确认替换后立即更新合成源与音色指标，再返回同一调音页。
...
- Riff 为 6 弦 × 16 步，单格循环关闭 / 普通 / 重音 / 闷音；手机端以 I–IV 四个小节页展示，每页只渲染 4 列，保证格子至少 44 px 且没有横向裁切。播放调度使用递归 `setTimeout`，每一步从 `riffRef.current` 读取最新格子和 BPM，因此播放中编辑会在播放头下一次到达时生效，不再使用启动播放时的旧闭包。90、120、150 BPM 均支持，普通 / 重音 / 闷音力度分别映射为 0.72、1.0、0.38。
