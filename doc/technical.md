...
> 本文依据 2026-07-21 的实际代码编写，覆盖内容评审台与玩家完整循环。最近上线提交：`b863d76`（原声模块平台）与 `65962ea`（琴墙缩略图弦钮可见性）。
...
- 乐器使用内联模块化 SVG，统一 `600 × 1200` 坐标系。玩家制琴使用互不混接的 `25.5 in bolt-on` 与 `24.75 in set-neck` 两个平台；`review.html` 另提供可自由校验的 `25.4 in acoustic flat-top` 原声平台。
...
src/
  ...
  ToneFoundry/
    ...
    guitar-system/modular*Platform.ts # 两个电吉他和一个原声平台的兼容矩阵与预设
    components/ModularAcousticGuitarSvg.tsx # 四种独立原声箱体、木桥、开放弦钮与面板饰面
...
- 正式玩家工单的两个电平台目前共有 11 种可玩琴体：bolt-on 的 slab、contour、offset、thinline、reverse，以及 set-neck 的 carved、centerblock、thin-horn、v-wing、angular、archtop。`buildRun.ts` 从对应兼容矩阵中抽取 2–3 项；更换琴体时会修正失配的拾音器与琴桥，最终保存 Guitar ID、五步等级和稀有分。`review.html` 的原声平台另有 grand-concert、grand-auditorium、dreadnought、super-jumbo 四种箱体，但尚未加入 `BuildPlatform` 的正式随机池。
- `buildTone.ts` 从平台母版出发，再按琴体、琴颈、拾音器、琴桥和饰面逐项叠加合成参数；`BODY_DELTAS` 为薄空心、反向偏移、V 翼、角型和拱面提供各自的共鸣 / 空间 / 驱动倾向。两个电平台各有 8 种可抽取饰面；评审原声平台提供 6 种可见面板饰面，同时改变木纹、外缘、包边、护板与漆面反射。`ToneEngine.playComparison` 播放固定 2.4 秒四音短句，便于前后候选直接比较。
...
- `PublicWall`、`GuitarWallDetail` 与 `AssemblyGuitarPreview` 都复用 `ModularGuitarPreview`，保证发布数据不会切换成静态截图。为避免弦钮在墙卡中缩小至不可辨，`ModularBoltOnNeckSvg` 与 `ModularSetNeckNeckSvg` 的弦钮包含扩大后的钥匙、金属轴、正面衬套与高光；`BuildRun.less` 将墙内 SVG 宽度设为 `86 px`、最大高度 `98%`。该修复同时作用于旧发布条目，因为条目保存的是模块配置而非图像。
...
- 英文是设计主语言；首次启动会依据 `navigator.language` 自动选择 `zh / en`，`localStorage.game_locale` 可明确覆盖该判断。入口使用 `AssemblyGuitarPreview` 的零确认状态呈现灰银工程轮廓和五模块提示，不会提前显示实体成琴。内部 `review.html` 继续保留自由拼装、十一把校准母版、缩放观察和音色表；`ModularAcousticGuitarSvg` 同时驱动 Grand Concert、Grand Auditorium、Dreadnought 与 Super Jumbo 的评审装配，独立维护箱体比例、3+3 开放弦钮琴头、桥型、rosette、品位与连续弦路，`ToneEngine` / `pluckModel` 以 94–132 Hz 箱体峰值和不同空气腔模态区分校准母版的动态。
...
- 新增模块或兼容关系：`guitar-system/modular*Platform.ts` 与对应 `components/Modular*GuitarSvg.tsx` / `components/modules/`。
