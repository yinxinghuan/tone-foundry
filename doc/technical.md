...
- `audio/effects.ts` 定义可保存的 `boost / overdrive / chorus / tape-echo` 链，`EffectPedal.tsx` 使用拟物 SVG 渲染单块。`ToneEngine` 在琴体合成和全局空间混响之间按链顺序插入效果节点；Riff、比较句、标准试听和 Remix 后的草稿均读取同一 `effects` 数组。
...
- Riff 为 6 弦 × 16 步，单格循环关闭 / 普通 / 重音 / 闷音；手机端以 I–IV 四个小节页展示，每页只渲染 4 列，保证格子至少 44 px 且没有横向裁切。`RIFF_PRESETS` 内置 6 条具名起始节奏；成琴时随机注入一条，Riff Lab 的“换一条”会从其余预载节奏随机抽取。播放调度使用递归 `setTimeout`，每一步从 `riffRef.current` 读取最新格子和 BPM，因此播放中编辑会在播放头下一次到达时生效，不再使用启动播放时的旧闭包。90、120、150 BPM 均支持，普通 / 重音 / 闷音力度分别映射为 0.72、1.0、0.38。
...
- 琴墙卡片的 `onClick` 打开独立 `GuitarWallDetail`，而不是复用简化成琴页；详情包含可缩放生产 SVG、作者与主页入口、Guitar ID、稀有分、五部件清单、六维音色、First Riff BPM、播放/停止和 6×16 Riff 带。非本人作品可直接发起 Remix：复制其模块配置与 Riff 到新的本地草稿，生成新 Guitar ID，并在发布对象的 `remix` 来源字段中保留原作品、原琴和作者；墙卡与详情会显示这条来源关系。
