...
- **One-sentence visual thesis**：收藏级模块化 SVG 吉他置于暖暗制琴工作台，以产品摄影的克制光线、真实材料和精密硬件建立拟物可信度。
...
- **Playfield focal area**：制琴态手机上方 `52–58dvh` 为常驻吉他，下方 `42–48dvh` 为零件托盘；桌面左侧约 `58–64%` 为吉他，右侧为装配控制台。试音态手机上方 58–66% 为吉他与六弦直接交互区。
...
- **Attention path**：琴体轮廓 → 拾音器与桥 → 琴名与文化标签 → 六维音色 → 试听按钮。
...
- **Builder screen contract**：默认“制琴台”占满一个安全视口；琴架永远不因切换零件或文化配方离开屏幕。桌面为左琴右控，手机为上琴下托盘；右控 / 下托盘内部切换内容，不把五类选项纵向同时展开。
- **Builder hierarchy**：第一焦点是整把吉他；第二层是当前结构平台与正在编辑的部件；第三层才是兼容数量、锚点和 manifest。平台、零件分类、具体选项、配方四级不得同时使用相同强调度。
- **Cultural signature**：控制台结合 custom-shop 工单、琴盒铭牌、放大器刻度和零件抽屉编号；每个选项呈现部件类别编号与“已装配 / 不兼容”机械状态，不使用通用 SaaS 圆角胶囊。
...
- **Entry / idle**：首次打开无需登录即可看到一把完整 Blackguard Standard；声音未启用时有明确“触弦启声”提示。
